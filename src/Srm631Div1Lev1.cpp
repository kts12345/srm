...
namespace haskell {
auto zipwith3 = [](auto x1, auto x2, auto x3, auto op)
{
...
auto zipwith = [](auto x1, auto x2, auto op)
{
...
acc_t handler(const acc_t& acc, const event_t& nows)
{
  ...
  auto cnts2 = zipwith3(prevs, nows, cnts,
                       [](auto& p, auto& n, auto&c)->int {return (p == n) ? (c + 1) : 1; });
  auto maxs2 = zipwith(cnts2, maxs, [](auto a, auto b) { return std::max(a, b); });
